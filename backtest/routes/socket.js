...
module.exports = io => {
	io.on('connection', socket => {
		...
		socket.on('CHAT_GENERAL_MESSAGE', async msg => {
			...
			await query(`INSERT INTO chat(SENDER, RECEIVER, TEXT, CREATED_AT) SELECT f.id, t.user_id, '${msg.message}', '${msg.created_at}' FROM users f, users t WHERE f.id = '${socket.user_id}' and t.id = '${msg.targetId}';`)
